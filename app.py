...
def editar_requerimiento(req_id):
    ...
    if request.method == "POST":
        ...
        try:  # <<<--- AÑADIDO
            ...
            if "archivo" in request.files:
                ...
                if file.filename != "":
                    if archivo_antiguo_path and os.path.exists(
                        os.path.join(app.config["UPLOAD_FOLDER"], archivo_antiguo_path)
                    ):
                        os.remove(
                            os.path.join(
                                app.config["UPLOAD_FOLDER"], archivo_antiguo_path
                            )
                        )
                    filename = secure_filename(file.filename)
                    file.save(os.path.join(app.config["UPLOAD_FOLDER"], filename))
            ...
            # Volvemos a renderizar la página de edición par "
...
def crear_orden(req_id):
    ...
    if request.method == "POST":
        ...
        db.commit()
        flash(
            f'Orden creada para el requerimiento '
